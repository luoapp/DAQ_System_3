...
//-----------------------------------------------------------------------
  ...
  buffa = malloc(NDATA*sizeof(unsigned int) );
  ...
  int ix, ix_f, ix_b, bx1, ix2, ix3, ix4,  x1[1024], x2[1024];
  ...
  for ( ix_f = 0; ix_f <file_num; ix_f++)
  
    {
      ...
      int chan_offset = (int)(ix_f * ((float)UDELdaqfilesize / UDELdaqblocksize / 2) +ix_b * UDELdaqblocksize) % nchans;
      printf("ix_b=%d chan_offset = %d\n",ix_b,chan_offset);
      
      /*for(ix2=0; ix2<win_width-1; ix2 ++ )
      {
        XSetForeground(display, gc, white.pixel);
        ix3 = (int)(((float)ix2) / win_width * (NDATA- nchans + 1 ) / nchans) * nchans + uctl.xwin_chan;
        ix4 = (int)(((float)ix2+1.0) / win_width * (NDATA - nchans +1) / nchans) * nchans + uctl.xwin_chan ;
        ...
        {
          ix3 = 0  + (int)(((float)ix2) / win_width * (NDATA- nchans + 1 ) / nchans) * nchans + uctl.xwin_chan;
          x1[ix2] = (int)(-(rdata2[ix3] - uctl.xwin_offset) * win_height /uctl.xwin_fs  + win_height/2);
        }
        for ( ix2=0; ix2<win_width-1; ix2++)
        {
          XSetForeground(display, gc, white.pixel);
          XDrawLine(display, win, gc, ix2, x2[ix2], ix2+1, x2[ix2+1]);
          XSetForeground(display, gc, black.pixel);
          XDrawLine(display, win, gc, ix2, x1[ix2], ix2+1, x1[ix2+1]);
        }
        memcpy(x2, x1, win_width * sizeof(int));  
  ...
  char t_end[64];
  udeltime( t_end );
  printf( "start: %s stop: %s",t_start, t_end );
