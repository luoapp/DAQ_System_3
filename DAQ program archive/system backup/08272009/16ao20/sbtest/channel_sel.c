// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/16AO20/16AO20_Linux_2.x.x.x_GSC_DN/sbtest/channel_sel.c $
// $Rev: 2014 $
...
static int _service_test(int fd)
{
	...
	else if ((chans != 6) && (chans != 12) && (chans != 20))
	{
		errs++;
		printf(	"FAIL <---  (channel quantity query error: %ld)\n",
				(long) chans);
	}
	else
	{
		errs			+= ao20_initialize_quiet(fd);
		mask			= 0xFFFFFFFF >> (32 - chans);
		list[0].mask	= mask;
		list[1].mask	= mask;
		...
		for (i = 0; i < chans; i++)
		{
			...
			list[0].arg		= value;
			list[0].value	= value;
			list[1].arg		= mask ^ value;
			list[1].value	= mask ^ value;
...
static int _function_test(int fd)
{
	...
	// the time it takes for the buffer to empty with both 2 channels enabled
	// and only one channel enabled.
...
/******************************************************************************
*
*	Function:	channel_sel_test
*
*	Purpose:
*
*		Perform a test of the IOCTL service AO20_IOCTL_CHANNEL_SEL.
*
*	Arguments:
*
...
int channel_sel_test(int fd)
{
	...
	gsc_label("AO20_IOCTL_CHANNEL_SEL");
