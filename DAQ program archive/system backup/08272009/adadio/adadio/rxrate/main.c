// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/ADADIO/ADADIO_Linux_3.x.x.x_GSC_DN/rxrate/main.c $
// $Rev: 1620 $
...
static int _parse_args(int argc, char** argv)
{
	...
	printf("USAGE: rxrate <-c> <-C> <-dma> <-dmdma> <-m#> <-n#> <-pio> <-r#> <#>\n");
	...
	printf("   -dmdma   Trabsfer data using Demand Mode DMA mode data transfer.\n");
...
static int _perform_tests(int fd)
{
	...
	errs	+= adadio_fsamp_report_all(fd, -1, 1);

	errs	+= rx_rate_test(fd, _receive_mb);
...
int main(int argc, char** argv)
{
	...
	{
		test	= time(NULL);
		printf("rxrate - Receive Rate Sample Application (Version %s)\n", VERSION);
