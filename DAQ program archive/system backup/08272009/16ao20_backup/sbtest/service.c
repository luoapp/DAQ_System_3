// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/16AO20/16AO20_Linux_2.x.x.x_GSC_DN/sbtest/service.c $
// $Rev: 2014 $
...
static int _reg_mod(int fd, int index, __u32 reg, __u32 mask, __u32 value)
{
	...
	for (;;)	// A convenience loop.
	{
		...
		parm.value	= value;;
		parm.mask	= mask;
		i			= ioctl(fd, AO20_IOCTL_REG_MOD, &parm);
		...
		printf("AO20_IOCTL_REG_MOD, errno %d)\n", errno);
		break;
	}
...
static int _reg_read(int fd, int index, __u32 reg)
{
	...
	for (;;)	// A convenience loop.
	{
		...
		if (i == -1)
		{
			...
			printf("AO20_IOCTL_REG_READ, errno %d)\n", errno);
			break;
		}
