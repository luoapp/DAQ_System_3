...
extern long BOARDSAMPLINGCLOCK;

void udelwritedatatofile( __u16 * datas, long int datasize){
	static int isnewfile = 1;
	static long int currentfilesize = 0;
	//FILE *daq_file = fopen("/daqsbc/data/data.dat", "a+");
	static FILE *daq_file;
	if (datas == NULL)
	{
		if (isnewfile == 0) 
		{
	...
	if ( isnewfile )
	{
		daq_file = udeldaqfileinit((DaqBoard *)NULL, 0); 
		isnewfile = 0;
	}
	//fwrite(data[buf.mtext[0]-48], BUFFERSIZE, 1, daq_file);
	fwrite(datas, datasize, 1, daq_file);
	currentfilesize += datasize;
	if (currentfilesize >= UDELdaqfilesize)
	{
		fclose(daq_file);
		isnewfile = 1;
	}
...
//  ptm = gmtime ( &rawtime );
  ...
  sprintf ( daqfilename,"/daqsbc/data/%s.daq", fileheader.UTCtime );
  printf( "%s\n", daqfilename);
...
{
	if (boardtype == gscADADIO)
	{
		pb->ctl.cfg.clock = 20000000;
		pb->ctl.ictl.nchans = 8;
		
	}
	else 
	{
		printf("udelboardinit error !\n");
		exit(1);
	}
}
...
void udelargparser(int argc, char **argv, DaqBoard * pb, UDELctl * pu )
{
  ...
  pb->ctl.ictl.ndiv	= (int)( pb->ctl.cfg.clock / pu->input_nominal_sr );
  pb->ctl.ictl.clockrate = ((float)pb->ctl.cfg.clock) / pb->ctl.ictl.ndiv;
...
//  char* font_name = "*-helvetica-*-12-*"; /* font to use for drawing text.   */
  ...
  win = create_simple_window(display, win_width, win_height, 0, 420);
